...
type AnnotationItem struct {
	...
	GdsSeqno             int         `orm:"column(gds_seqno)" description:"商品序号"`
	PutrecSeqno          int         `orm:"column(putrec_seqno)" description:"备案序号(对应底账序号）"`
	GdsMtno              string      `orm:"column(gds_mtno);size(32)" description:"商品料号"`
	Gdecd                string      `orm:"column(gdecd);size(10)" description:"商品编码"`
	GdsNm                string      `orm:"column(gds_nm);size(512)" description:"商品名称"`
	GdsSpcfModelDesc     string      `orm:"column(gds_spcf_model_desc);size(512)" description:"商品规格型号"`
	DclUnitcd            string      `orm:"column(dcl_unitcd);size(3);null" description:"申报计量单位"`
	DclUnitcdName        string      `orm:"column(dcl_unitcd_name);size(100);null" description:"申报计量单位名称"`
	LawfUnitcd           string      `orm:"column(lawf_unitcd);size(3);null" description:"法定计量单位"`
	LawfUnitcdName       string      `orm:"column(lawf_unitcd_name);size(100);null" description:"法定计量单位名称"`
	...
	Natcd                string      `orm:"column(natcd);size(3)" description:"原产国(地区)"`
	NatcdName            string      `orm:"column(natcd_name);size(100)" description:"原产国(地区)名称"`
	DclUprcAmt           float64     `orm:"column(dcl_uprc_amt);digits(19);decimals(4)" description:"企业申报单价"`
	DclTotalAmt          float64     `orm:"column(dcl_total_amt);digits(19);decimals(2)" description:"企业申报总价"`
	UsdStatTotalAmt      float64     `orm:"column(usd_stat_total_amt);null;digits(25);decimals(5)" description:"美元统计总金额"`
	DclCurrcd            string      `orm:"column(dcl_currcd);size(3)" description:"币制"`
	DclCurrcdName        string      `orm:"column(dcl_currcd_name);size(100)" description:"币制名称"`
	LawfQty              float64     `orm:"column(lawf_qty);digits(19);decimals(5)" description:"法定数量"`
	...
	DclQty               float64     `orm:"column(dcl_qty);digits(19);decimals(5)" description:"申报数量"`
	...
	LvyrlfModecd         string      `orm:"column(lvyrlf_modecd);size(6)" description:"征免方式"`
	LvyrlfModecdName     string      `orm:"column(lvyrlf_modecd_name);size(100)" description:"征免方式名称"`
	...
	DestinationNatcd     string      `orm:"column(destination_natcd);size(3)" description:"最终目的国"`
	DestinationNatcdName string      `orm:"column(destination_natcd_name);size(100)" description:"最终目的国名称"`
	ModfMarkcd           string      `orm:"column(modf_markcd);size(1)" description:"修改标志"`
	...
	ModfMarkcdName       string      `orm:"column(modf_markcd_name);size(100);null" description:"修改标志 0-未修改 1-修改 2-删除 3-增加"`
