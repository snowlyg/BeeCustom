		...
		admin = {
			...
			table_radio_click: function () {
				$(document).on("click",
							   ...
							   function (e) {
								   if ($(e.target).hasClass("layui-table-col-special") || $(e.target)
									   .closest(".layui-table-col-special").length) {
									   return false;
								   }
								   let index = $(this).attr('data-index'),
									   ...
									   radioCell = tableDiv.find("tr[data-index=" + index + "]")
														   .find("td div.laytable-cell-radio div.layui-form-radio i");
								   if (checkCell.length) {
									   checkCell.click();
								   }
								   if (radioCell.length) {
			...
			commodity_storehouse_save() {
				const data = layui.table.checkStatus('commodity_storehouse_table').data;
				if (data.length == 0) {
			...
			async manual_no_save() {
				const check_data = layui.table.checkStatus('manual_no_list_table').data;
				...
				if ($(dom).val().trim()) {
					...
					for (let item of data) {
						if (item.serial == $(dom).val()) {
							...
							$("#g_qty").focus();
							const code_data = await admin.get(`/hs_code/lists?limit=0&search=${item.hs_code}`);
			...
			net_wt_blur(dom) {
				if ($(dom).val().trim()) {
					if (parseFloat($(dom).val().trim()) > parseFloat($("#GrossWet").val().trim())) {
						$(dom).focus();
			...
			async trade_code_change(dom) {
				setTimeout(() => {
					if (!($(dom).val().trim())) {
						...
						const index = company_list.findIndex((item) => item.value == $(dom).val());
			...
			manual_no_change
				...
				setTimeout(async () => {
					if ($(dom).val().trim()) {
						const data = await admin.get(`/order/account_manual?limit=0&search=${$(dom).val()}`);
						if (data.length === 0) {
			...
			putrec_seqno_change: async function (dom, flag, handBookId) {
				...
				if ($(dom).val().trim()) {
					...
					let handbookgoods = await admin.post("/handbook/get_hand_book_good_by_hand_book_id",
			...
			g_unit_name_change(dom) {
				setTimeout(() => {
					if ($("#first_unit").val() && $("#g_unit").val() == $("#first_unit").val() && $("#g_qty")
						.val() && !($("#first_qty").val())) {
						$("#first_qty").val($("#g_qty").val());
					}
					if ($("#second_unit").val() && $("#g_unit").val() == $("#second_unit").val() && $("#g_qty")
						.val() && !($("#second_qty").val())) {
			...
			g_qty_change(dom) {
				if (admin.cutZero($("#decl_price").val().trim()) > 0 && admin.cutZero(
					...
					admin.g_qty_change_index = layer.open({
															  ...
															  success: function (layero, index) { //层创建完毕时触发
																  ...
																  $("body")
																	  .on("click", "#selectDeclPrice", function () {
																		  admin.calculationDeclPrice();
																	  });
																  $("body")
																	  .on("keydown", "#selectDeclPrice", function (e) {
																  ...
																  $("body")
																	  .on("click", "#selectDeclTotal", function () {
																		  admin.calculationDeclTotal();
																	  });
																  $("body")
																	  .on("keydown", "#selectDeclTotal", function (e) {
																		  const eCode = e.keyCode ? e.keyCode : e.which ? e.which : e.charCode;
																		  if (eCode == 37) { //<-键
				...
				if ($("#decl_price").val().trim() && !($("#decl_total").val().trim()) && admin.cutZero(
			...
			totalChange() {
				...
				if (gQty && gQty != "0" && declTotal && !isNaN(gQty) && !isNaN(declTotal)) {
			...
			dcl_qty_change(dom) {
				...
				if ($("#DclUprcAmt").val().trim() && !($("#DclTotalAmt").val().trim()) && admin.cutZero(
				...
				if ($("#DclUnitcd").val().trim()) {
					...
					if ($("#DclUnitcd").val().trim() == $("#SecdLawfUnitcd").val().trim()) {
			...
			decToDecimal(num1, num2, digit, calculationType, roundingMode) {
				...
				let resultList = admin.decCalculation(calculationDatas, digits, calculationTypes, roundingModes, '1');
				return resultList[0];
			}
			,
			...
			decCalculation(calculationDatas, digits, calculationTypes, roundingModes, isZeroNoShow) {
				...
				for (var i = 0; i < calculationDatas.length; i++) {
					...
					let calculationDataMap = admin.calculation(calculationData, calculationTypes[i],
				...
				for (var i = 1; i < calculations.length; i++) {
					...
					if (calculationType == "0") {
						...
						if (calculation == "0") {
							...
							bigDecimalResult = bigDecimalResult.divide(bigDecimal, parseInt(digit),
			...
			elements_change(dom) {
				...
				for (let item in data) {
					if (item == 0) {
						const data_filter = admin.brand_type_data.filter(t => t.id == data[item]);
						$("#val0").val(data_filter[0].id);
						$("#val0Name").val(data_filter[0].value);
					} else if (item == 1) {
						const data_filter = admin.export_benefits_data.filter(t => t.id == data[item]);
...
//表体商品编号弹窗
			async openGoodsWindow(e) {
				...
				if (eCode == 13 || !eCode) {
					...
					const data = await admin.get(`/hs_code/lists?limit=0&search=${codeTs}`);
...
//tips计算总价/成交数量合计/法定第一数量/法定第二数量
			...
			async base_clearance_data_auto(obj, clearance_update_times) {
				if ((layui.data(obj.data_name).data === undefined || layui.data(
					obj.index).data === undefined) || clearance_update_times[obj.name] != layui.data(obj.index).data) {
...
//报关整合申报自动完成汇总
			...
			async annotations_auto(layuicomplete) {
				...
				const destination_natcd = {
					layuicomplete: layuicomplete,
					elem: '#DestinationNatcdName',
					hideElem: '#DestinationNatcd',
					type: "国别地区代码",
					cache: false,
				};
				...
				const secd_lawf_unitcd = {
					layuicomplete: layuicomplete,
					elem: '#SecdLawfUnitcdName',
					hideElem: '#SecdLawfUnitcd',
					type: "计量单位代码",
					cache: false,
				};
				...
				const exempting_method = {
					layuicomplete: layuicomplete,
					elem: '#LvyrlfModecdName',
					hideElem: '#LvyrlfModecd',
					type: "征减免税方式代码",
					cache: false,
				};
			...
			DecListGNoCheck(data, decData, InvtList) {
				...
				if (gNo != "") {
					if (gNo <= decData.length) {
						...
						if ("1" != $("#invt_type")
							.val() && ("" == _decRow.destination_natcd || undefined == _decRow.destination_natcd)) {
							bool = data.gdecd == _decRow.gdecd && data.gds_nm == _decRow.gds_nm &&
						...
						if (bool) {
							...
							layer.open({
										   title: "提示",
										   content: "表体序号[" + data.gds_seqno + "]：报关单序号已经存在，但是与对应的报关商品信息归并条件不同，不能进行归并",
										   icon: 2
									   });
							return false;
						}
			...
			combineDec(decList, tmpDecList) {
				...
				if (!!decList.dcl_total_amt) {
					let __t = new BigDecimal(decList.dcl_total_amt.toString())
						.add(new BigDecimal(
							tmpDecList.dcl_total_amt.toString() === "" ? "0" : tmpDecList.dcl_total_amt.toString()))
				...
				if (!!decList.lawf_qty) {
					let __t = new BigDecimal(decList.lawf_qty.toString())
						.add(new BigDecimal(
							tmpDecList.lawf_qty.toString() === "" ? "0" : tmpDecList.lawf_qty.toString()))
						.setScale(5, MathContext.ROUND_HALF_UP).toString();
				...
				if (!!decList.secd_lawf_qty) {
					let __t = new BigDecimal(decList.secd_lawf_qty.toString())
						.add(new BigDecimal(
							tmpDecList.secd_lawf_qty.toString() === "" ? "0" : tmpDecList.secd_lawf_qty.toString()))
						.setScale(5, MathContext.ROUND_HALF_UP).toString();
				...
				if (!!tmpDecList.dcl_qty && tmpDecList.dcl_qty.toString() !== "0" && !!tmpDecList.dcl_total_amt) {
					let __t = new BigDecimal(tmpDecList.dcl_total_amt).divide(
						new BigDecimal(tmpDecList.dcl_qty.toString()), 5, MathContext.ROUND_HALF_UP)
							...
							if (decListData.length === 0) {
								...
								for (let j = 0; j < decListData.length; j++) {
									...
									if (l.gdecd == _tmp.gdecd &&
										...
										if (l.entry_gds_seqno && l.entry_gds_seqno != _tmp.entry_gds_seqno) {
...
//核注清单-办理记录
			async annotation_jilu_click(dom, type) {
				const id = $(dom).data("id");
				layer.load(2);
				const data_order = await admin.get(`/annotation/${type}/${id}/annotation_logs`);
				layer.closeAll('loading');
				const data = {
					data_order: data_order
				};
				...
				layui.form.on('submit(order_add_entry_submit)', async (form) => {
					try {
						const data = await admin.post(
							`/order/${admin.add_entry.flag}/${admin.add_entry.id}/add_entry_id`, form.field);
						if (data.status === 1) {
							setTimeout(() => {
								layer.closeAll();
							}, 2000)
				...
				layui.laypage.render({
										 ...
										 jump: async function (obj, first,OrdrIndexRequestData) {
											 if (!first) {
												 ...
												 ListDatas = await admin.post(`${url}`, OrderIndexRequestListData,
																			  true);
												 $("#order-i-table tbody").remove();
												 if (ListDatas.total === 0) {
													 $("#order-i-table").append(
...
//异地报关
			is_other_change(dom) {
				...
				if (is_other) {
					$("#agent_code_scc").val("").removeAttr("lay-verify").removeAttr("lay-vertype")
										.removeClass("required").attr("disabled", "disabled");
					$("#agent_code").val("").removeAttr("lay-verify").removeAttr("lay-vertype").removeClass("required")
									.attr("disabled", "disabled");
					$("#decl_ciq_code").val("").attr("disabled", "disabled");
					$("#agent_name").val("").removeAttr("lay-verify").removeAttr("lay-vertype").removeClass("required")
									.attr("disabled", "disabled");
				} else {
					$("#agent_code_scc").val("914419007962277606").attr("lay-verify", "required")
										.attr("lay-vertype", "tips").addClass("required").removeAttr("disabled");
					$("#agent_code").val("4419986507").attr("lay-verify", "required").attr("lay-vertype", "tips")
									.addClass("required").removeAttr("disabled");
					$("#decl_ciq_code").val("4400510052").removeAttr("disabled");
					$("#agent_name").val("广东东华报关服务有限公司").attr("lay-verify", "required").attr("lay-vertype", "tips")
									.addClass("required").removeAttr("disabled");
				}
			}
			,
                    ...
                    <form class="layui-form" onkeydown="if(event.keyCode===13){return false}">
                        ...
                        <div class="manual_no_btn">
                            <button class="layui-btn custom-create_btn" lay-submit lay-filter="sync_annotation_to_order_save"
                                    id="sync_annotation_to_order_save" type="button" data-flag="${$(dom).data("flag")}"
...
//同步关务通--填写平台单证号-保存
			sync_annotation_to_order_save(dom) {
				layui.form.on('submit(sync_annotation_to_order_save)', async (data) => {
					try {
						const res = admin.post(`/order/${$(dom).data("flag")}/sync_annotation_to_order`, data.field);
						if (res.status) {
							setTimeout(() => {
								layer.closeAll();
							}, 2000);
			...
			theme: function (options) {
				var theme = setter.theme,
					...
					styleText = laytpl([
										   ...
										   //选中色
										   , '.layui-nav-tree .layui-this,', '.layui-nav-tree .layui-this>a,', '.layui-nav-tree .layui-nav-child dd.layui-this,', '.layui-form-select dl dd.layui-this,', '.layui-nav-tree .layui-nav-child dd.layui-this a', '{background-color:{{d.color.selected}} !important;}'

										   , '.layui-form-radio>i:hover, .layui-form-radioed>i', '{color:{{d.color.selected}} !important;}'
