    ...
    let $ = layui.jquery,
        ...
        admin = {
            ...
            table_radio_click: function () {
                $(document).on("click", ".layui-table-body table.layui-table tbody tr, .layui-table-body table.layui-table tbody tr td", function (e) {
                    if ($(e.target).hasClass("layui-table-col-special") || $(e.target).closest(".layui-table-col-special").length) {
                        return false;
                    }
            ...
            dec_users_commodity_click() {
                ...
                layui.table.render({
                    ...
                    autoSort: false,
                    url: `/history_item/list?sortedBy=desc&orderBy=created_at&trade_code=${$("#trade_code").val()}&i_e_flag=${i_e_flag}`,
                    response: {
                        countName: 'total'
                    },
                    toolbar: true,
            ...
            net_wt_blur(dom) {
                if ($(dom).val().trim()) {
                    if (parseFloat($(dom).val().trim()) > parseFloat($("#gross_wet").val().trim())) {
                        $(dom).focus();
            ...
            async bizop_etpsno_change(dom) {
                setTimeout(() => {
                    ...
                    const index = company_list.findIndex((item) => item.value == $(dom).val());
                    if (company_list[index].credit_code) {
                        $("#bizop_etps_sccd").val(company_list[index].credit_code);
                    }
                    if (company_list[index].name) {
                        $("#bizop_etps_nm").val(company_list[index].name);
                    }
                }, 150)
            },
            ...
            async rcvgd_etpsno_change(dom) {
                setTimeout(() => {
                    ...
                    const index = company_list.findIndex((item) => item.value == $(dom).val());
                    if (company_list[index].credit_code) {
                        $("#rvsngd_etps_sccd").val(company_list[index].credit_code);
                    }
                    if (company_list[index].name) {
                        $("#rcvgd_etps_nm").val(company_list[index].name);
                    }
                }, 150)
            },
            ...
            //监听进出口清单备案序号
            putrec_seqno_change: async function (dom) {
                setTimeout(() => {
                    if ($(dom).val().trim()) {
                        const flag = $(dom).data('flag');
            ...
            g_unit_name_change(dom) {
                setTimeout(() => {
                    ...
                    if ($("#second_unit").val() && $("#g_unit").val() == $("#second_unit").val() && $("#g_qty").val() && !($("#second_qty").val())) {
            ...
            g_qty_change(dom) {
                ...
                if ($("#decl_price").val().trim() && !($("#decl_total").val().trim()) && admin.cutZero($("#decl_price").val().trim()) > 0) {
                    admin.calculationDeclTotal();
                }
                if ($("#g_unit").val().trim()) {
            ...
            dcl_qty_change(dom) {
                if (admin.cutZero($("#dcl_uprc_amt").val().trim()) > 0 && admin.cutZero($("#dcl_total_amt").val().trim()) > 0) {
                    admin.dcl_qty_change_index = layer.open({
                        ...
                        success: function (layero, index) { //层创建完毕时触发
                            ...
                            $("body").on("click", "#selectDeclPrice_ann", function () {
                                console.log(1);
                                admin.calculationDeclPrice_ann();
                            });
                            $("body").on("keydown", "#selectDeclPrice_ann", function (e) {
                                const eCode = e.keyCode ? e.keyCode : e.which ? e.which : e.charCode;
                                if (eCode == 39) { //->键
                            ...
                            $("body").on("click", "#selectDeclTotal_ann", function () {
                                admin.calculationDeclTotal_ann();
                            });
                            $("body").on("keydown", "#selectDeclTotal_ann", function (e) {
                                const eCode = e.keyCode ? e.keyCode : e.which ? e.which : e.charCode;
                                if (eCode == 37) { //<-键
                ...
                if ($("#dcl_uprc_amt").val().trim() && !($("#dcl_total_amt").val().trim()) && admin.cutZero($("#dcl_uprc_amt").val().trim()) > 0) {
                    admin.calculationDeclTotal_ann();
                }
                if ($("#dcl_unitcd").val().trim()) {
                    if ($("#dcl_unitcd").val().trim() == $("#lawf_unitcd").val().trim()) {
                        $("#lawf_qty").val($(dom).val().trim());
                    }
                    if ($("#dcl_unitcd").val().trim() == $("#secd_lawf_unitcd").val().trim()) {
                        $("#secd_lawf_qty").val($(dom).val().trim());
                    }
                }
            },
            ...
            DecListGNoCheck(data, decData, InvtList) {
                ...
                if (gNo != "") {
                    if (gNo <= decData.length) {
                        ...
                        if ("1" != $("#invt_type").val() && ("" == _decRow.destination_natcd || undefined == _decRow.destination_natcd)) {
                        ...
                        } else if ("1" == $("#invt_type").val() && ("" == _decRow.destination_natcd || undefined == _decRow.destination_natcd)) {
