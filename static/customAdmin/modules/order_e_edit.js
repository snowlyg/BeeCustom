layui.define(function (exports) {
    layui.use(['form', 'admin', 'table', 'AutoComplete', 'laydate', 'laytpl', 'upload'], async function () {
            ...
            if (order_i_edit_data.order_status_string == '复核不通过') {
                if (order_i_edit_data.reject_logs.length > 0) {
                    if (order_i_edit_data.reject_logs[order_i_edit_data.reject_logs.length - 1].recheck_error_input_ids) {
                        for (let item of order_i_edit_data.reject_logs[order_i_edit_data.reject_logs.length - 1].recheck_error_input_ids) {
        ...
        if ($("#order_i_edit_data").val()) {
            ...
            if (order_i_edit_data.order_status_string == '复核不通过') {
                if (order_i_edit_data.reject_logs.length > 0) {
                    $("#order_application_review span").text('重新申请复核');
                    if (order_i_edit_data.reject_logs[order_i_edit_data.reject_logs.length - 1].recheck_error_input_ids) {
                        for (let item of order_i_edit_data.reject_logs[order_i_edit_data.reject_logs.length - 1].recheck_error_input_ids) {
                            if (item.index) {
                                $(`div[lay-id='order_pros'] .layui-table-body tr[data-index=${item.index - 1}]`).css('background-color', 'rgba(255, 235, 236, 1)');
                            } else {
                                if (item.id == 'promise_itmes0' || item.id == 'promise_itmes1' || item.id == 'promise_itmes2' || item.id == 'type1') {
                                    $(`#${item.id}`).next('.layui-form-checkbox').css('background-color', 'rgba(255, 235, 236, 1)');
                                } else if (item.id == 'containers') {
                                    $(`div[lay-id='order_containers_table'] .layui-table-body table`).css('background-color', 'rgba(255, 235, 236, 1)');
                                } else if (item.id == 'documents') {
                                    $(`div[lay-id='order_documents_table'] .layui-table-body table`).css('background-color', 'rgba(255, 235, 236, 1)');
                                } else {
                                    $(`#${item.id}`).addClass('is_check_fail');
                                }
                            }
                        }
                    }
            ...
            async function get_copy_data(){
                ...
                if(copy_all_data.status){
                    setTimeout(async () => {
                        ...
                        admin.order_containers_delete_ids=reset_order_containers;
        ...
        form.verify({
            ...
            net_wt(value, item) {
                if (!(value.trim())) {
                    return "请填写净重";
                } else if (parseFloat(value.trim()) > parseFloat($("#gross_wet").val().trim())) {
            ...
            dec_g_no(value, item) {
                let gNo = 1;
                for (let pros_item of order_pros_data) {
                    if (pros_item.g_no == value) {
                        gNo = 0
                    }
                }
                if (gNo) {
                ...
                if (order_documents[order_documents_index - 1].eco_relations.length > 0) {
                    for (let documents_item of order_documents[order_documents_index - 1].eco_relations) {
                        if (documents_item.dec_g_no == value) {
                            dec_g_no = 1
                        }
                    }
                    if (dec_g_no) {
