layui.define(function (exports) {
    layui.use(['form', 'admin', 'table', 'AutoComplete', 'laydate', 'laytpl', 'upload'], async function () {
        ...
        const cusIEFlag = admin.cusIEFlag = 'E';
        ...
        $('body').on('click', '#contr_item_save', async function () {
            ...
            if (!data[0].moneyunit) {
                if (order_pros_data.length) {
                    console.log(order_pros_data[0]);
                    console.log(order_pros_data[0].origin_country_name);
                    $("#trade_curr_name").val(order_pros_data[0].trade_curr_name);
                    $("#destination_country_name").val(order_pros_data[0].destination_country_name);
                    $("#origin_country_name").val(order_pros_data[0].origin_country_name);
                    $("#district_code_name").val(order_pros_data[0].district_code_name);
                    $("#duty_mode_name").val(order_pros_data[0].duty_mode_name);
                }
            } else {
            ...
            switch (obj.event) {
                ...
                case 'batch_edit':
                    ...
                    auto_fn({
                        ...
                        listDirection: false,
                        id: ['#destination_country_name_batch'],
                        after: ['#destination_country_batch']
                    });
                    /**自动完成--批量修改--境内目的地代码**/
                    auto_fn({
        ...
        form.on('submit(distribute_submit)', async (data) => {
            await admin.post(`/order/e/${order_id}/distribute`, data.field);
            setTimeout(() => {
        ...
        table.on('tool(print_lists)', async function (obj) {
            switch (obj.event) {
                case 'preview':
                    ...
                    if (obj.data.code == 'local_5') {
                        if (obj.data.is_enclosure) {
                            const preview_data = await admin.get(`/order/e/pdf/${obj.data.pdf_id}/show`);
                            window.open(preview_data);
                        } else {
                            window.open(`/order/e/${order_id}/downloads/prints?file_type=${obj.data.id}`);
                        }
                    } else {
                        if (obj.data.is_enclosure) {
                            const preview_data = await admin.get(`/order/e/pdf/${obj.data.pdf_id}/show`);
                            ...
                            if (obj.data.code == '00000004') {
                                ...
                                layer.confirm('是否需要修改内容？', {
                                    ...
                                    layer.close(index);
                                    window.open(`/order/e/${order_id}/downloads/prints?file_type=${obj.data.id}${orientation_save}`);
                                });
                            }
                            ...
                            if (obj.data.code == 'local_6') {
                                ...
                                layer.confirm('是否需要修改内容？', {
                                    ...
                                    layer.load(2);
                                    const driver_data = await admin.get(`/order/e/${order_id}/order_driver_paper?voy_no=${order_i_edit_data.voy_no}`);
                                    /** 货物申报司机纸打印弹窗表格渲染 */
                ...
                case 'download':
                    ...
                    if (obj.data.code == 'local_5') {
                        if (obj.data.is_enclosure) {
                            window.open(`/order/e/pdf/${obj.data.pdf_id}/downloads`);
                        } else {
                            window.open(`/order/e/${order_id}/downloads/prints?file_type=${obj.data.id}`);
                        }
                    } else {
                ...
                case 'enclosure_save':
                    ...
                    if (obj.data.code == 'local_5') {
                        layer.load(2);
                        const save_pdf_data = await admin.get(`/order/e/${order_id}/downloads/save_pdf?file_type=${obj.data.id}${orientation_save}`, 'show');
                        layer.closeAll('loading');
                ...
                case 'reload_save':
                    ...
                    if (obj.data.code == 'local_5') {
                        layer.load(2);
                        const reload_save_pdf_data = await admin.get(`/order/e/${order_id}/downloads/save_pdf?file_type=${obj.data.id}&pdf_id=${obj.data.pdf_id}${orientation_save}`, 'show');
                        layer.closeAll('loading');
        ...
        form.on("submit(print_edit_contract_save)", async (data) => {
            ...
            window.open(encodeURI(`/order/e/${order_id}/downloads/prints?file_type=${file_type_id}${orientation_save}${str}`));
        });
        /**打印前修改-发票-预览**/
        form.on("submit(print_edit_invoice_save)", async (data) => {
            ...
            layer.close(print_edit_index);
            window.open(`/order/e/${order_id}/downloads/prints?file_type=${file_type_id}${orientation_save}${str}`);
        });
        /**打印前修改-司机纸-预览**/
        ...
        form.on("submit(print_save_contract_save)", async (data) => {
            ...
            layer.load(2);
            if (save_pdf_id) {
                save_pdf_data = await admin.get(`/order/e/${order_id}/downloads/save_pdf?file_type=${file_type_id}&pdf_id=${save_pdf_id}${orientation_save}`, 'show');
            } else {
                save_pdf_data = await admin.get(encodeURI(`/order/e/${order_id}/downloads/save_pdf?file_type=${file_type_id}${orientation_save}${str}`), 'show');
            }
            layer.closeAll('loading');
            if (save_pdf_data.status) {
                await admin.getPdf_e(order_id);
        ...
        if ($("#order_i_edit_data").val()) {
            ...
            laytpl($("#order_fail_template").html()).render(order_i_edit_data, function (html) {
                $("#order_fail").html(html)
            });
        ...
        /**审核通过**/
        $("body").on("click", "#order_save_pass", async function () {
            if (order_i_edit_data.order_status_string == '审核通过' || order_i_edit_data.order_status_string == '待派单') {
                layer.closeAll('loading');
        ...
        $("body").on("click", "#order_init", async function () {
            ...
            table.render({
                ...
                , skin: 'line'
                , url: `/template/lists?sortedBy=desc&orderBy=created_at`
                , response: {
                    countName: 'total'
                }
                , cols: [[
        ...
        /**驳回**/
        $("body").on("click", "#order_audit_reject", function () {
            if (order_i_edit_data.order_status_string == '审核不通过') {
                layer.closeAll('loading');
            ...
            layer.open({
                ...
                shadeClose: true,
                area: admin.screen() < 2 ? ['80%', '300px'] : ['650px', '340px'],
                content: $('#order_audit_reject_template').html()
            });
            form.render();
        });
        $("body").on("input", "#order_audit_reject_remark", function () {
            $("#remark_audit_reject_number span").text($(this).val().length);
        });
        /**驳回保存**/
        form.on('submit(order_audit_reject_submit)', async (data) => {
            layer.load(2);
            if (order_i_edit_data.order_status_string == '待审核' || order_i_edit_data.order_status_string == '审核中') {
                const res = await admin.post(`/order/e/${order_id}/audit_reject`, data.field);
                if (res.status) {
                    setTimeout(() => {
                        layer.closeAll();
                        window.location.reload();
        ...
        $("body").on("click", "#order_cancel", function () {
            layer.confirm('真的要取消订单吗？一旦取消，订单将被关闭！', { title: '提示' }, async (index) => {
                layer.load(2);
                const data = await admin.post(`/order/e/${order_id}/cancel`);
                setTimeout(() => {
                    layer.closeAll();
                    window.location.reload();
                }, 1000);
        ...
        $("body").on("click", "#order_recall", function () {
            layer.confirm('真的要撤回订单吗？', { title: '撤回订单' }, async (index) => {
                layer.load(2);
                const data = await admin.post(`/order/e/${order_id}/recheck_cancel`);
                setTimeout(() => {
                    layer.closeAll();
                    window.location.reload();
                }, 1000);
        ...
        $("body").on("click", "#order_reminder", async function () {
            try {
                const data = await admin.post(`/order/e/${order_id}/press`);
            } catch (e) {
        ...
        /**开始制单/继续制单/修改/改单**/
        $(document).on("click", "#order_c_maker", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/e/${order_id}/#/id=${order_id}`
                , text = `出口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**改单**/
        $(document).on("click", "#order_switch_bill", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/e/${order_id}/get_change/#/id=${order_id}`
                , text = `改单出口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**修改--审核员**/
        $(document).on("click", "#order_edit", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/e/create/${order_id}`
                , text = `修改出口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**修改--制单员**/
        $(document).on("click", "#order_edit_maker", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/e/${order_id}`
                , text = `制单/修改出口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**提交复核/重新提交复核**/
        $("body").on("click", "#order_application_review, #order_application_review_reload", async function () {
            if (order_i_edit_data.order_status_string == '复核中') {
                layer.closeAll('loading');
                return layer.msg('该订单已经提交审核');
            }
            try {
                const data = await admin.get(`/order/e/${order_id}/recheck_again`, 'show');
                if (data.status) {
                    setTimeout(() => {
                        layer.closeAll();
                        window.location.reload();
        ...
        $("body").on("click", "#order_check", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/e/${order_id}/get_recheck/#/id=${order_id}`
                , text = `复核出口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**申请改单**/
        let order_application_change_index;
        $("body").on("click", "#order_application_change", async function () {
        ...
        form.on('submit(order_application_change_note_submit)', async (data) => {
            await admin.post(`/order/e/${order_id}/for_change`, data.field);
            layer.close(order_application_change_index);
        });
        /**差错登记**/
        let order_error_logging_index;
        $("body").on("click", "#order_error_logging", async function () {
        ...
        form.on('submit(order_error_logging_submit)', async (data) => {
            try {
                const res = await admin.post(`/order/e/${order_id}/errors`, data.field);
                if (res.status) {
                    setTimeout(() => {
                        layer.close(order_error_logging_index);
                    }, 1000);
                }
            } catch (e) {
                return layer.msg('接口错误！', {
        ...
        /**重新开启订单**/
        $("body").on("click", "#order_reopen", async function () {
            try {
                const res = await admin.post(`/order/e/${order_id}/restart`);
                if (res.status) {
                    setTimeout(() => {
                        layer.closeAll();
                        window.location.reload();
        ...
        /**提交到单一窗口-选择附件**/
        $("body").on("click", "#order_submit_single", async function () {
            try {
                layer.open({
                    type: 1,
                    title: '暂存',
        ...
        $('body').on('click', '#order_enclosure_check', async function () {
            const data = table.checkStatus('order_enclosure_check_lists').data;
            ...
            try {
                const edoc_codes = data.map((item) => item.edoc_code);
                const data_check = await admin.post(`/order/e/${order_id}/create_xml`, {
                    edoc_codes: edoc_codes,
                    decl_trn_rel: decl_trn_rel
                });
        ...
        /**还原订单**/
        $("body").on("click", "#order_recycle_restore", async function () {
            try {
                const res = await admin.get(`/order/recycle/${order_id}/restore`, 'show');
            } catch (e) {
                return layer.msg('接口错误！', {
                    offset: '15px'
                    , icon: 2
        ...
        $("input,textarea,checkbox").attr("disabled", "disabled");
        $("#non-disabled-btn").click();
        $("#decListShow").click();
        $("div[lay-id='order_documents_table'] button").attr("disabled", "disabled").addClass('order_pros_btn_disabled');
        $("div[lay-id='order_containers_table'] button").attr("disabled", "disabled").addClass('order_pros_btn_disabled');
        try {
