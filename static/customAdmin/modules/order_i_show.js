layui.define(function (exports) {
    layui.use(['form', 'admin', 'table', 'AutoComplete', 'laydate', 'laytpl', 'upload'], async function () {
        ...
        $('body').on('click', '#contr_item_save', async function () {
            ...
            if (!data[0].moneyunit) {
                if (order_pros_data.length) {
                    ...
                    $("#dest_code_name").val(order_pros_data[0].dest_code_name);
        ...
        form.on('submit(declaration_next)', function (data) {
            declaration_save_btn(data);
            next_dec();
            $('#declaration_next_look').attr('id', 'declaration_next_look');
            $('#declaration_last_look').attr('id', 'declaration_last_look');
        });
                        ...
                        if (obj.data.is_enclosure) {
                            ...
                            if (obj.data.code == 'local_6') {
                                ...
                                layer.confirm('是否需要修改内容？', {
                                    ...
                                    $("#district_code_name_driver").val(driver_data.first_pros.district_code_name);
                                    ...
                                    $("#origin_country_name_driver").val(driver_data.first_pros.origin_country_name);
        ...
        form.on('submit(order_audit_reject_submit)', async (data) => {
            ...
            if (order_i_edit_data.order_status_string == '待审核' || order_i_edit_data.order_status_string == '审核中') {
                const res = await admin.post(`/order/i/${order_id}/audit_reject`, data.field);
                if (res.status) {
                    setTimeout(() => {
                        layer.closeAll();
                        window.location.reload();
        ...
        $("body").on("click", "#order_cancel", function () {
            layer.confirm('真的要取消订单吗？一旦取消，订单将被关闭！', { title: '提示' }, async (index) => {
                layer.load(2);
                const data = await admin.post(`/order/i/${order_id}/cancel`);
                setTimeout(() => {
                    layer.closeAll();
                    window.location.reload();
                }, 1000);
        ...
        $("body").on("click", "#order_recall", function () {
            layer.confirm('真的要撤回订单吗？', { title: '撤回订单' }, async (index) => {
                layer.load(2);
                const data = await admin.post(`/order/i/${order_id}/recheck_cancel`);
                setTimeout(() => {
                    layer.closeAll();
                    window.location.reload();
                }, 1000);
        ...
        $("body").on("click", "#order_reminder", async function () {
            try {
                const data = await admin.post(`/order/i/${order_id}/press`);
            } catch (e) {
        ...
        $(document).on("click", "#order_c_maker", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/i/${order_id}/#/id=${order_id}`
                , text = `进口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**改单**/
        $(document).on("click", "#order_switch_bill", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/i/${order_id}/get_change/#/id=${order_id}`
                , text = `改单进口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**修改--审核员**/
        $(document).on("click", "#order_edit", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/i/create/${order_id}`
                , text = `修改进口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**修改--制单员**/
        $(document).on("click", "#order_edit_maker", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/i/${order_id}`
                , text = `制单/修改进口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**提交复核/重新提交复核**/
        $("body").on("click", "#order_application_review, #order_application_review_reload", async function () {
            ...
            try {
                const data = await admin.get(`/order/i/${order_id}/recheck_again`, 'show');
                if (data.status) {
                    setTimeout(() => {
                        layer.closeAll();
                        window.location.reload();
        ...
        $("body").on("click", "#order_check", async function () {
            const topLayui = parent === self ? layui : top.layui
                , href = `/order/i/${order_id}/get_recheck/#/id=${order_id}`
                , text = `复核进口货物申报${order_i_edit_data.client_seq_no}`;
            topLayui.index.openTabsPage(href, text);
        });
        /**申请改单**/
        ...
        form.on('submit(order_application_change_note_submit)', async (data) => {
            await admin.post(`/order/i/${order_id}/for_change`, data.field);
        ...
        form.on('submit(order_error_logging_submit)', async (data) => {
            try {
                const res = await admin.post(`/order/i/${order_id}/errors`, data.field);
                if (res.status) {
                    setTimeout(() => {
        ...
        $("body").on("click", "#order_reopen", async function () {
            try {
                const res = await admin.post(`/order/i/${order_id}/restart`);
                if (res.status) {
                    setTimeout(() => {
                        layer.closeAll();
                        window.location.reload();
        ...
        $("body").on("click", "#order_submit_single", async function () {
            try {
                ...
                table.render({
                    ...
                    , skin: 'line'
                    , height: 450
                    , url: `/order/i/${order_id}/pdf/lists?sortedBy=desc&orderBy=created_at&is_eco_relations=1`
                    , response: {
                        countName: 'total'
                    }
                    , cols: [[
        ...
        $('body').on('click', '#order_enclosure_check', async function () {
            ...
            try {
                ...
                const data_check = await admin.post(`/order/i/${order_id}/create_xml`, {
