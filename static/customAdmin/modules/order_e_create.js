    ...
    layui.use(['form', 'admin', 'table', 'AutoComplete', 'laydate', 'laytpl', 'upload'], async function () {
        ...
        async function order_i_add_back_filling(order_i_edit_data) {
            if (parent.layui.admin.get_iframe_index()) {
                ...
                if (order_i_edit_data.order_status_string != '已完成') {
                    const lay_id = parent.layui.admin.get_iframe_index().attr("lay-id").replace("/create", "");
                    parent.layui.admin.get_iframe_index().attr("lay-id", `${lay_id}/${order_id}/#/id=${order_id}`);
                    parent.layui.admin.get_iframe_index().attr("lay-attr", `${lay_id}/${order_id}/#/id=${order_id}`);
                }
            }
