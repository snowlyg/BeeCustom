layui.define(function (exports) {
    layui.use(['form', 'admin', 'table', 'AutoComplete', 'laydate', 'laytpl', 'upload'], async function () {
        ...
        table.on('toolbar(order_pros)', async function (obj) {
            ...
            switch (obj.event) {
                ...
                case 'again':
                    ...
                    $('#declaration_next_look').attr('id','declaration_next');
                    $('#declaration_last_look').attr('id','declaration_last');
        ...
        form.on('submit(order_save)', async (data) => {
            ...
            const order_save_data = await admin.patch(`/order/i/${order_id}`, data.field);
        ...
        $('body').on('click','#order_copy_data',function(){
            ...
            async function get_copy_data(){
                let copy_all_data = await admin.post(`/order/i/copy_order_data/`,{client_seq_no:$('#copy_order_id').val()});
...
/*
                        admin.order_quas_vin_delete_ids = reset_order_pro;
                        console.log(admin.order_quas_vin_delete_ids);*/
                            }, 1000);
                        }
            }
        })
        ...
        table.on('tool(order_enclosure_check_lists)', async function (obj) {
            ...
            if (obj.event === 'preview') {
                const preview_data = await admin.get(`/order/i/pdf/${data.id}/show`);
                window.open(preview_data);
            }
        });
        /**提交到单一窗口-提交**/
        ...
        form.on('submit(order_save_reload_submit)', async (data) => {
            ...
            const order_save_data = await admin.post(`/order/i/re_create_xml`, data.field);
            setTimeout(() => {
                layer.closeAll()
            }, 2000);
        });
